...
var require_schedule = __commonJS({
  "node_modules/@netlify/functions/dist/lib/schedule.js"(exports) {
    ...
    var schedule2 = (cron, handler2) => handler2;
...
// netlify/functions/callWeatherApi.ts
var callWeatherApi_exports = {};
__export(callWeatherApi_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(callWeatherApi_exports);
var import_functions = __toESM(require_main());
var fetch = require_lib3();
var myHandler = async (event, context) => {
  try {
    const response = await fetch("https://api.api-ninjas.com/v1/weather?city=hanoi", {
      headers: { "X-Api-Key": "vlq7dkvbzK2lPyKj4Dzr8A==VIEjzOKKidA6NUUm" },
      contentType: "application/json"
    });
    if (!response.ok) {
      throw new Error(`API WEATHER request failed with status: ${response.status}`);
    }
    ...
    if (!responseApp.ok) {
      throw new Error(`API SMARTHOME request failed with status: ${responseApp.status}`);
    }
...
var handler = (0, import_functions.schedule)("@hourly", myHandler);
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  handler
});
//# sourceMappingURL=callWeatherApi.js.map
